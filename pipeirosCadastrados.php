...
if ($estaLogado == "SIM" && !isset($active)) {
    ...
    if (isset($_GET["busca"])) {
    ...
    } else { // caso não haja busca, mostra todos os pipeiros
        // ########### PAGINAÇÃO #############//
        ...
        $piperoCadastrado->setOrderTable("id_pipeiro LIMIT $inicio, $quantPipeiro"); // selecionando pipeiros dentro da paginação

        $pagina = new Pagination($pg, $quantPipeiro, $paginacao->CountPipeiroTodos());
        $piperoCadastrado->selectPipeiroTodos();
        $smarty->assign("total", $paginacao->CountPipeiroTodos());
        $smarty->assign("local", "");
