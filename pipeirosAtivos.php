...
if ($estaLogado == "SIM" && !isset($active)) {
    ...
    if (isset($_SESSION["erroPipeiro"])) {
        if ($_SESSION["erroPipeiro"] == "excluir") {
            ...
            $smarty->assign("erroPipeiro", "<div class='alert alert-success' role='alert'>Pipeiro desativado com sucesso!</div>");
        }
    } else {
...
// ######### FIM DA PAGINAÇÃO ###########// 
        ...
        $pagina = new Pagination($pg, $quantPipeiro, $paginacao->CountPipeiroAtivo());
        $piperoCadastrado->selectPipeiroAtivo();
        $smarty->assign("total", $paginacao->CountPipeiroAtivo());
        $smarty->assign("local", "sim");
